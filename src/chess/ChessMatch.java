...
public class ChessMatch { // Partida de xadrez
	...
	public ChessPiece perfomChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		ChessPiece movedPiece = (ChessPiece)board.piece(target);//peça que se moveu foi a de destino 
		
		// #specialmove promotion 
		promoted = null;
		if (movedPiece instanceof Pawn) { // testar se chegou no final
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == null) { // significa que ele mudou de coluna, foi pra diagonal e não capturou a peça, significa en passan
				Position pawnPosition;
				
				if (p.getColor() == Color.WHITE) { // a peça que vai ser capturada esta embaixo do meu peão branco
					pawnPosition = new Position(target.getRow() + 1, target.getColumn());
				}
				else {
					pawnPosition = new Position(target.getRow() - 1, target.getColumn());
				}
				capturedPiece = board.removePiece(pawnPosition); // vai remover a peça que estiver capturada
 				capturedPieces.add(capturedPiece); // adicionando as peças capturadas
 				piecesOnTheBoard.remove(capturedPiece); // removendo a peça do meu tabuleiro
			}
		}
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) { 
				ChessPiece pawn = (ChessPiece)board.removePiece(target); // tirei ela do lugar errado que ela estava
				Position pawnPosition;
